...
class HostEnvironment
{
    ...
    predicate Valid()
        reads this;
    {
           constants != null && ok != null && files != null && ow != null
    }
...
class HostConstants
{
    ...
    // extended
    static method {:extern} AskForOverwrite(dest: array<char>, ghost env: HostEnvironment) returns (result: bool)
        requires env.Valid()
        requires env.ok.ok()
        requires !env.ow.ow()
        requires dest[..] in env.files.state()
        modifies env.ow
        ensures env.ow.ow() == result
}
...
class OverwriteState {
    constructor {:axiom} () requires false;
    function {:axiom} ow(): bool reads this;
}
